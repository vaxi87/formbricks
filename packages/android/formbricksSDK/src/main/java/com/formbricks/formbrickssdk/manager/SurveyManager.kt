...
import com.formbricks.formbrickssdk.model.upload.StorageData
import com.formbricks.formbrickssdk.model.user.Display
import com.formbricks.formbrickssdk.network.FormbricksFileUploadService
...
object SurveyManager {
    ...
    private val displayTimer = Timer()
    ...
    fun postResponse(response: SurveyResponse?, surveyId: String?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                if (responseId != null) {
                    FormbricksApi.updateSurveyResponse(responseId!!, resp)
                } else {
                    responseId = FormbricksApi.postSurveyResponse(id, UserManager.userId, displayId, resp).getOrThrow().data.id
                    if (UserManager.userId == null) {
                        UserManager.anonymousOnResponse(id)
                    }
                }
    ...
    fun createNewDisplay(surveyId: String?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = FormbricksApi.postDisplay(UserManager.userId, id).getOrThrow()
     ...
     */
    fun uploadFile(file: FileData, surveyId: String?, uploadId: String, listener: FileUploadListener) {
        val id = surveyId.guard {
            Timber.tag("SurveyManager").e("Survey id is mandatory to set.")
            return
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = FormbricksApi.fetchUploadUrl(file.name, file.type, null, id).getOrThrow()
                reallyUploadFile(file, id, response.data, uploadId, listener)
            } catch (e: Exception) {
                Timber.tag("SurveyManager").e(e, "Unable to fetch file upload url.")
            }
        }
    }
     ...
     */
    private fun reallyUploadFile(file: FileData, surveyId: String, storageData: StorageData, uploadId: String, listener: FileUploadListener) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val urlElements = storageData.signedUrl.replace("http://localhost:3000", Formbricks.appUrl).split("/")
                val path = urlElements.last()
                val url = urlElements.dropLast(1).joinToString("/") + "/"
                val body = FileUploadBody.create(file, storageData, surveyId)
                FormbricksFileUploadService(url, Formbricks.loggingEnabled).uploadFile(path, body).getOrThrow()
                listener.fileUploaded(storageData.fileUrl, uploadId)
            } catch (e: Exception) {
                Timber.tag("SurveyManager").e(e, "Unable to upload file.")
            }
        }
    }
