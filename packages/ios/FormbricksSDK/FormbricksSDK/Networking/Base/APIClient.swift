...
class APIClient<Request: CodableRequest>: Operation, @unchecked Sendable {
    ...
    override func main() {
        ...
        guard let requestEndPoint = setEnvironmentIdInPath(request.requestEndPoint) else {
            completion?(.failure(FormbricksSDKError(type: .sdkIsNotInitialized)))
            return
        }
...
private extension APIClient {
    ...
    func setEnvironmentIdInPath(_ path: String) -> String? {
        guard let environmentId = Formbricks.environmentId else {
            return nil
        }
        return path.replacingOccurrences(of: "{environmentId}", with: environmentId)
    }
}
